...
-i <flag> --hmi <flag>  a flag for activating the graphical interface [default: False]
...
class WumpusHMI(Tk):
    ...
    def updateLoop(self):
        ...
        if(end_flag):
            ...
            self.hist_cumuls.append(self.cumul_reward_)
            self.hist_time.append(self.time_step_)
            self.reset()
            self.environment.reset()

        self.updateWorld()
        platform.after(self.DELTA_TIME, self.updateLoop)
...
if __name__ == "__main__":
    ...
    eps   = [0.01, 0.1, 0.3]#, 0.3, 0.6, 0.9]
    tau   = [1, 3, 10]
    #agents = [('Agent()', 'random'), ('AG.PrudentAgent()', 'Prudent')]
    #agents = [('AG.epsGreedyAgent('+str(ep)+', AG.XYBSF(grid_size,n_flash))', 'XYBSF eps-Greedy ({})'.format(ep)) for ep in eps] 
    #agents = [('AG.softmaxAgent('+str(t)+', AG.BSF(n_flash))',   'BSF Softmax ({})'.format(t)) for t in tau ]
    agents  = [('AG.UCBAgent(AG.XYBSF(grid_size,n_flash))', 'XYBSF UCB'),('AG.UCBAgent(AG.ABSF(n_flash))', 'ABSF UCB'),('Agent()', 'random')]
    HR = [];
    HC = [];
    for agent_, name in agents:
        ...
        for run in range(20):
            ...
            if my_args["--hmi"]=="True":
                platform = WumpusHMI(agent,my_args)
                for i in range(int(my_args["--max_n_episode"])):
                    print('Episode '+repr(i+1))
                    while True:
                        platform.updateLoop()
                        platform.update()
                        if platform.time_step_ == 0:
                            break
                platform.destroy()
                ...
                for i in range(int(my_args["--max_n_episode"])):
                    ...
                    while True:
                        ...
                        if platform.time_step_ == 0:
                            break
            HR[-1].append(platform.hist_rewards)
            HC[-1].append(platform.hist_cumuls)
    
    plt.figure(1,figsize=(8,4))
    for i in range(len(agents)):
        HR[i]=[item for sublist in HR[i] for item in sublist]
        print ('Average reward/step for "{}": {:.2f}'.format(agents[i][1],np.sum(HR[i])/len(HR[i])))
        print ('Average cumulative reward/episode for "{}": {:.2f}'.format(agents[i][1],np.mean(HC[i])))
        plt.figure(1)
        m = np.mean(HC[i],axis=0)
        err=np.vstack((m-np.min(HC[i],axis=0),np.max(HC[i],axis=0)))
        plt.errorbar(range(int(my_args["--max_n_episode"])), np.mean(HC[i],axis=0), label=agents[i][1], yerr=err)

    plt.legend(loc=2,ncol=3,bbox_to_anchor=(0., 1., 1., .1),prop={'size':8})
    plt.xlabel('Plays')
    plt.ylabel('Average reward')
    plt.ylim([-200,200])
    plt.tight_layout()
    plt.savefig('../figures/UCB.jpeg')
    plt.show()
